...
def bp_is_displayed(player:Player, cbp:str)->bool:
    if cbp=="IMAGES" and (player.participant.vars["treatment"] == 0 or player.participant.vars["type"] < 2): return False
    return True
...
def bp_before_next_page(player:Player,timeout_happened:bool, cbp:str, next_cbp:str) -> None:
    if cbp=="MORALQUEST":
        for v in getattr(C,cbp+'_VARS'):
            setattr(player,v,getattr(player,v)-1)
...
def bp_vars_for_template(player:Player,cbp:str) -> Dict[str,Any]:
    if cbp=="MORALQUEST":
        if not "treatment" in player.participant.vars:
            player.participant.vars["treatment"]=2
        ndiffquests=4
        qlist=[]
        cats=[]
        for i in range(ndiffquests):
            qlist.append(dict(
                q=getattr(C,cbp+"_LIST")[i],
                n=i+1
            ))
        for i in range(len(getattr(C,cbp+"_CATS"))):
            cats.append(dict(
                cat=getattr(C,cbp+"_CATS")[i],
                fr=getattr(C,cbp+"_CATSFR")[i],
            ))
        return dict(ndiffquests=ndiffquests,qlist=qlist,cats=cats)
    return {}
